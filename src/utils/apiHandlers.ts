 ...
 */
export async function handleFirstPass(request: FirstPassRequest): Promise<NextResponse> {
  const { 
    chunk, 
    chunkIndex, 
    totalChunks, 
 ...
 */
export async function handleSecondPass(request: SecondPassRequest): Promise<NextResponse> {
  const { 
    allEntries,
    totalPages,
    exampleIndex = '',
 ...
 */
function parseIndexResponse(responseText: string, startPage: number, endPage: number): IndexEntry[] {
  try {
 ...
 */
function parseRefinedResponse(responseText: string, fallbackEntries: IndexEntry[]): IndexEntry[] {
  try {
    // Check if response is too large (likely too verbose)
    if (responseText.length > 80000) { // ~80KB limit
      console.warn(`Response is very large (${responseText.length} chars), may contain excess verbosity`);
    }
    ...
    validatedEntries.forEach((entry: any) => {
      if (entry.subentries && entry.subentries.length > 0) {
        entry.subentries.sort((a: any, b: any) => a.term.toLowerCase().localeCompare(b.term.toLowerCase()));
      }
    });
