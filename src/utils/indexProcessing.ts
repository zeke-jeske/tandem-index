 ...
 */
export function mergeIndexEntries(existingEntries: IndexEntry[], newEntries: IndexEntry[]): IndexEntry[] {
  ...
  newEntries.forEach(newEntry => {
    ...
    if (existingIndex >= 0) {
      ...
      if (newEntry.subentries && newEntry.subentries.length > 0) {
        ...
        newEntry.subentries.forEach(newSubentry => {
          ...
          if (existingSubIndex >= 0) {
            ...
            // Apply Chicago Manual of Style page range formatting
            existing.subentries![existingSubIndex].pageNumbers = formatPageRanges(updatedSubPages);
          } else {
            // Add new subentry
      ...
      const formattedEntry = {
        ...newEntry,
        pageNumbers: newEntry.pageNumbers ? formatPageRanges(newEntry.pageNumbers) : newEntry.pageNumbers,
 ...
 */
export function determineProcessingStrategy(pageCount: number, textLength: number): boolean {
  ...
  return pageCount < SINGLE_PASS_PAGE_LIMIT && textLength < SINGLE_PASS_CHAR_LIMIT;
}
