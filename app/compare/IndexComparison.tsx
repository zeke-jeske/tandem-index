          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <div className="text-sm text-gray-500 mt-1">
                {index1.length} characters, ~
                {Math.round(index1.split('\n').filter((l) => l.trim()).length)}{' '}
                entries
              </div>
            </div>
            ...
            <div>
              ...
              />
              <div className="text-sm text-gray-500 mt-1">
                {index2.length} characters, ~
                {Math.round(index2.split('\n').filter((l) => l.trim()).length)}{' '}
                entries
              </div>
            </div>
          </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
              ...
              <div
                className={`p-4 rounded-lg mb-6 text-center ${
                  result.winner === 'index1'
                    ? 'bg-blue-100 border border-blue-300'
                    : result.winner === 'index2'
              ...
              >
                ...
                <div className="text-lg">
                  <strong>{index1Title}:</strong> {result.overallScore.index1}
                  /100 |<strong> {index2Title}:</strong>{' '}
                  {result.overallScore.index2}/100
                </div>
              </div>
                ...
                {Object.entries(result.criteria).map(([criterion, scores]) => (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <div
                        className={`text-sm ${scores.index1 > scores.index2 ? 'font-bold text-blue-600' : 'text-gray-600'}`}
                      >
                        {index1Title}: {scores.index1}/10
                      </div>
                      <div
                        className={`text-sm ${scores.index2 > scores.index1 ? 'font-bold text-purple-600' : 'text-gray-600'}`}
                      >
