...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!documentPageCount || typeof documentPageCount !== 'number' || documentPageCount <= 0) {
        return NextResponse.json(
    ...
    const trimmedText = fullText.length > 30000 
...
Remember this document is ${documentPageCount} pages total, so your page numbers should be between 1 and ${documentPageCount}.`;
    ...
    if (!responseText) {
      console.log('No text content found in response:', JSON.stringify(response));
      return NextResponse.json(
    ...
    predictedPages.results = predictedPages.results.map((result: Partial<PredictedPageResult>, index: number): PredictedPageResult => {
      // If pageNumber is missing or not a number, set a default
      if (!result.pageNumber || typeof result.pageNumber !== 'number') {
        console.log(`Warning: Missing or invalid page number for result ${index}`);
        return {
          ...result,
    ...
    console.log('Parsed prediction details:', JSON.stringify(predictedPages, null, 2));
